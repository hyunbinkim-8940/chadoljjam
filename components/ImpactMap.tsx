...
export const ImpactMap: React.FC<ImpactMapProps> = ({ data, selectedId, onSelect }) => {
    ...
    useEffect(() => {
        ...
        g.append("g")
            ...
            .attr("color", "rgba(0,0,0,0.05)") // Darker grid for light mode
        ...
        g.selectAll("circle")
            ...
            .attr("stroke", d => d.id === selectedId ? "#fbbf24" : "rgba(255,255,255,0.8)")
    ...
    return (
        <GlassCard className="flex-1 min-h-[400px] flex flex-col relative overflow-hidden p-0">
             <div ref={containerRef} className="w-full h-full relative">
                 <h3 className="text-slate-400 font-bold tracking-[0.2em] text-sm absolute top-6 left-6 z-10 pointer-events-none" style={{ fontFamily: 'Space Grotesk' }}>IMPACT MAP</h3>
                 
                 {/* Legend */}
                 <div className="absolute top-6 right-6 z-10 flex gap-4 bg-white/80 p-2.5 rounded-xl backdrop-blur-md border border-white/60 shadow-lg">
                     <div className="flex items-center gap-2">
                         <div className="w-2.5 h-2.5 rounded-full bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.6)]"></div>
                         <span className="text-[10px] text-slate-600 font-medium">ATOM</span>
                     </div>
                     <div className="flex items-center gap-2">
                         <div className="w-2.5 h-2.5 rounded-full bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.6)]"></div>
                         <span className="text-[10px] text-slate-600 font-medium">ATOMONE</span>
                     </div>
                     <div className="flex items-center gap-2">
                         <div className="w-2.5 h-2.5 rounded-full bg-purple-500 shadow-[0_0_8px_rgba(168,85,247,0.6)]"></div>
                         <span className="text-[10px] text-slate-600 font-medium">Mixed</span>
                     </div>
                 </div>

                 {/* Controls */}
                 <div className="absolute bottom-6 right-6 z-10 flex flex-col gap-2">
                     <button className="p-2.5 bg-white/50 hover:bg-white rounded-lg border border-slate-200 text-slate-500 hover:text-slate-900 transition-all backdrop-blur-sm shadow-sm">
                         <ZoomIn size={16} />
                     </button>
                     <button className="p-2.5 bg-white/50 hover:bg-white rounded-lg border border-slate-200 text-slate-500 hover:text-slate-900 transition-all backdrop-blur-sm shadow-sm">
                         <ZoomOut size={16} />
