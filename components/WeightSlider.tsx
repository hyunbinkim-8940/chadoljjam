...
export const WeightSlider: React.FC<WeightSliderProps> = ({ weights, onChange, disabled }) => {
    ...
    return (
        <div className={`relative h-full w-2.5 mx-auto ${disabled ? 'opacity-50' : ''}`}>
            {/* Track Background */}
            <div ref={trackRef} className="absolute inset-0 bg-slate-200 rounded-full w-full border border-slate-300 shadow-inner">
                {/* Slot A (Bottom) */}
                <div 
                    className="absolute bottom-0 w-full bg-indigo-500 rounded-b-full transition-all duration-75 shadow-[0_0_10px_rgba(99,102,241,0.3)]"
                    style={{ height: `${h1}%` }}
                />
                {/* Slot B (Middle) */}
                <div 
                    className="absolute w-full bg-pink-500 transition-all duration-75 shadow-[0_0_10px_rgba(236,72,153,0.3)]"
                    style={{ bottom: `${h1}%`, height: `${weights[1]}%` }}
                />
                {/* Slot C (Top) */}
                <div 
                    className="absolute top-0 w-full bg-emerald-500 rounded-t-full transition-all duration-75 shadow-[0_0_10px_rgba(16,185,129,0.3)]"
            ...
            {/* Handle 1 (A/B) */}
            <div 
                className={`absolute w-5 h-5 bg-white border-2 border-slate-400 rounded-full -left-1.5 shadow-md flex items-center justify-center cursor-row-resize z-10 hover:scale-110 transition-transform ${disabled ? 'cursor-not-allowed hidden' : ''}`}
            ...
            >
             ...
             {/* Handle Label 1 */}
             {!disabled && (
                 <div className="absolute left-5 text-[10px] font-bold text-slate-500 pointer-events-none drop-shadow-sm" style={{ bottom: `${h1}%`, transform: 'translateY(50%)' }}>
                     {Math.round(h1)}%
                 </div>
             )}
            ...
            >
             ...
             {/* Handle Label 2 */}
             {!disabled && (
                <div className="absolute left-5 text-[10px] font-bold text-slate-500 pointer-events-none drop-shadow-sm" style={{ bottom: `${h2}%`, transform: 'translateY(50%)' }}>
                    {Math.round(h2)}%
                </div>
            )}
        </div>
